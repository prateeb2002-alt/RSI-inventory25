...
// ── Load & render placeholder ─────────────────────────────────
// The real loadItems() is defined at the bottom of this file
// so it can also handle QR code redirect after items are fetched.
...
function saveItem() {
  ...
  var promise = editingId ? doUpdateItem(editingId, data) : doAddItem(data);
...
function confirmDel(id, name) {
  ...
  doDeleteItem(id).then(function(res) {
    _hideSpinner();
...
function openQr(id) {
  ...
  if (typeof QRCode !== 'undefined') {
    ...
    div.innerHTML = '<p style="color:#ef4444;font-size:13px">QR library not loaded. Needs internet.</p>';
  }
  _showModal('qrModal');
}
...
// ── QR scan redirect ──────────────────────────────────────────
// Reads ?edit=<id> from the URL (placed there by the QR code).
// Waits for items to fully load from Google Sheets FIRST,
// then opens the edit modal for that specific item.
...
(function(){
  ...
  if (editId) {
    ...
    // Remove ?edit=... from the browser address bar cleanly
    history.replaceState({}, '', window.location.pathname);
  }
}());

// ── Init ──────────────────────────────────────────────────────
// Override loadItems to also handle QR redirect after load
function loadItems() {
  _showSpinner('Loading items…');
  doGetItems().then(function(res) {
    _hideSpinner();
    if (!res.success) {
      showToast('Could not load items: ' + (res.message || 'error'), 'toast-del');
      allItems = [];
    } else {
      allItems = res.items || [];
    }
    renderTable();
    // If we arrived via QR code scan, open the edit modal now
    // that items are loaded and the correct item can be found
    if (_pendingEditId) {
      var id = _pendingEditId;
      _pendingEditId = null;
      setTimeout(function(){ openEdit(id); }, 200);
    }
  });
}
